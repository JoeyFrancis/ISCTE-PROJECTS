 ...
 **
 ** Aluno: Nº: a98647  Nome: João Monteiro
...
void init_ipc() {
    ...
    sucesso("C1) Fila de Mensagens com a Key %x aberta com o ID %i",IPC_KEY,msg_id);
...
void cria_mensagem() {
    ...
    printf("\nInsira o seu número de utente\n");
    scanf(" %d", &mensagem.dados.num_utente);
    printf("\nInsira o seu nome\n");
    scanf("\n%99[^\n]s\n",mensagem.dados.nome);
    sucesso("C2.1) Dados Cidadão: %d, %s\n",mensagem.dados.num_utente,mensagem.dados.nome);
...
void envia_mensagem_servidor() {
    ...
    int enviar=msgsnd(msg_id,&mensagem,sizeof(mensagem.dados),0);
    exit_on_error(enviar,"Não é possível enviar mensagem para o servidor");
    sucesso("Mensagem para o servidor enviada");
...
void espera_resposta_servidor() {
    ...
    int receber=msgrcv(msg_id,&resposta,sizeof(resposta.dados),getpid(),0);
    exit_on_error(receber,"Não é possível ler a resposta do servidor");
    sucesso("Servidor enviou resposta");
...
void pedido() {
    ...
    case NAOHAENFERMEIRO : erro("C5.1) Não existe enfermeiro na localidade do utente %d, %s", mensagem.dados.num_utente,mensagem.dados.nome);
        exit(1);
        break;
    case VACINADO : sucesso("C5.2) O utente %d, %s já foi vacinado", mensagem.dados.num_utente, mensagem.dados.nome);
        exit(0);
        break;
    case EMCURSO : sucesso("C5.3) A vacinação do utente %d, %s já está em curso", mensagem.dados.num_utente, mensagem.dados.nome);
    exit(0);
    break;
    case AGUARDAR : sucesso("C5.4) Utente %d, %s, por favor aguarde...", mensagem.dados.num_utente, mensagem.dados.nome);
    sleep(TEMPO_ESPERA);
    pedido();
    break;
    }
