...
void init_ipc() {
    ...
    exit_on_error(sem_id," S1) Semáforo com a Key definida já existe ou não pode ser criado");
    int valor_Mutex=semctl(sem_id, 0 , SETVAL, 1);
    exit_on_error(valor_Mutex,"S1) Semáforo com a Key definida não pode ser iniciado com o valor 1");
    ...
    sucesso(" S1) Criados elementos IPC com a Key 0x%x: MSG %d, SEM %d, SHM %d",IPC_KEY,msg_id,sem_id,shm_id);
...
void init_database() {
    ...
    // • Lê o ficheiro FILE_CIDADAOS e armazena o seu conteúdo na base de dados usando a função read_binary(), assim preenchendo os campos db->cidadaos e db->num_cidadaos. Se não o conseguir, dá erro e termina com exit status 1;
    size_t tamanho_cid;
     tamanho_cid=read_binary(FILE_CIDADAOS, &db->cidadaos, sizeof(Cidadao)*MAX_CIDADAOS);
    //sem_mutex_down();
    db->num_cidadaos=tamanho_cid/sizeof(Cidadao);
    //sem_mutex_up();
    // • Lê o ficheiro FILE_ENFERMEIROS e armazena o seu conteúdo na base de dados usando a função read_binary(), assim preenchendo os campos db->enfermeiros e db->num_enfermeiros. Se não o conseguir, dá erro e termina com exit status 1;
    size_t tamanho_enf;
    tamanho_enf=read_binary(FILE_ENFERMEIROS, &db->enfermeiros, sizeof(Enfermeiro)*MAX_ENFERMEIROS);
    sem_mutex_down();
    db->num_enfermeiros=tamanho_enf/sizeof(Enfermeiro);
    //sem_mutex_up();
...
void espera_mensagem_cidadao() {
    ...
    int receber_mensagem=msgrcv(msg_id,&mensagem,sizeof(mensagem.dados),1,0);
    exit_on_error(receber_mensagem,"Não é possível ler a mensagem do Cidadao");
...
void envia_resposta_cidadao() {
    ...
    int enviar_resposta=msgsnd(msg_id,&resposta,sizeof(resposta.dados),0);
    exit_on_error(enviar_resposta,"Não é possível enviar resposta para o cidadão");
...
void vacina() {
    ...
    // sucesso("S6.1) Criado um processo filho com PID_filho=%d", <PID_filho>);
     ...
     pid_t p_filho=fork();
     if (p_filho==0) {   // Processo FILHO
        // S6.2) O processo filho chama a função servidor_dedicado();
        sucesso("S6.1) Criado um processo filho com PID_filho=%d",p_filho);
       servidor_dedicado();
     } else if(p_filho >0) { // Processo PAI
        ...
        db->vagas[vaga_ativa].PID_filho=p_filho;
     }else{
         exit_on_error(p_filho,"S6.1) Não foi possível criar um novo processo");
     }
    debug(">");
}
...
void servidor_dedicado() {
    debug("<");
    int cidadao_cid= db->vagas[vaga_ativa].index_cidadao;
    int enfermeiro_enf=db->vagas[vaga_ativa].index_enfermeiro;
    ...
    db->enfermeiros[enfermeiro_enf].disponibilidade=0;
    //sem_mutex_up();
...
void liberta_vaga(int index_vaga) {
    ...
    sucesso("S9) A vaga com o index %d foi libertada",vaga_ativa);
    //sem_mutex_up();
...
void cancela_pedido() {
    ...
    if(numero_enf<0){
        ...
        sucesso(" S10.1) Foi encontrada a sessão do cidadão %d, %s na sala com o index %d",mensagem.dados.num_utente,mensagem.dados.nome,numero_enf);
    }
